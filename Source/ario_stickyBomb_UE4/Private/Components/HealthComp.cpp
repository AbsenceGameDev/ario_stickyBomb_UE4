  ...
  Super::BeginPlay();
	...
	if (GetOwnerRole() == ROLE_Authority) {
	  AActor* MyOwner = GetOwner();
		if (MyOwner) {
		  MyOwner->OnTakeAnyDamage.AddDynamic(this, &UHealthComp::HandleTakeAnyDamage);
		}
	}
...
void UHealthComp::Regenerate(float RegenAmount)
{
  ...
  UE_LOG(LogTemp, Log, TEXT("Health Changed: %s (+%s)"), *FString::SanitizeFloat(Health), *FString::SanitizeFloat(RegenAmount));

  OnHealthChanged.Broadcast(this, Health, -RegenAmount, nullptr, nullptr, nullptr);
}
